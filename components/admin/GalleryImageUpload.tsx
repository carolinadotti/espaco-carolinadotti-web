...
export default function GalleryImageUpload({ photoId, currentUrl, onSuccess }: GalleryImageUploadProps) {
  ...
  async function handleConfirm() {
    ...
    try {
      ...
      fd.append("type", `gallery-${photoId}`)
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="flex items-center gap-4">
        <div className="w-24 h-24 rounded overflow-hidden bg-muted border shrink-0">
          {currentUrl ? (
      ...
      <p className="text-xs text-muted-foreground">Proporção 1:1. Será convertida para WebP otimizado.</p>

      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="sm:max-w-xl">
          <DialogHeader><DialogTitle>Recortar foto da galeria</DialogTitle></DialogHeader>
          {src && (
            <div className="relative bg-black rounded overflow-hidden" style={{ height: 340 }}>
              <Cropper
                image={src} crop={crop} zoom={zoom} aspect={1}
